export function getMetadataJoins() {
  return `JOIN metadata_titles ON metadata_main.title_id = metadata_titles.title_id
      ...
      LEFT JOIN metadata_term_map_omit_deleted ON metadata_term_map_omit_deleted.episode_id = metadata_main.episode_id
      LEFT JOIN thesaurus_terms ON thesaurus_terms.term_id = metadata_term_map_omit_deleted.term_id`;
}
